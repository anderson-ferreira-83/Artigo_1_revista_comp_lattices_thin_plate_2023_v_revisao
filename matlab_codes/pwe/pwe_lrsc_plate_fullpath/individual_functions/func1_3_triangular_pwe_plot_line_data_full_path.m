function func1_3_triangular_pwe_plot_line_data_full_path = triangular_dispersion(nmax_f,i_fi_f,i_ff_f,a_f,he_f,E_f,gamma_f,ro_f,ni_f,fr_f,eta_p_f)
%disp('Parametros - Rede Triangular')
% Numero imaginario
im=sqrt(-1);
...
for i=-nmax:nmax
    for j=-nmax:nmax
        G = [G;i*(2*pi/a1) (i-2*j)*(2*pi/a2)*(1/sqrt(3))];
    end
end
% Trajetorias
%----------------------
gamma   =            [0 0];
X       = (pi/lat_a)*[4/3   0];
M       = (pi/lat_a)*[1     1/sqrt(3)];
X_l     = (pi/lat_a)*[2/3   2/sqrt(3)];
M_l     = (pi/lat_a)*[0     2/sqrt(3)];
%----------------------
...
label_file_w_kk = sprintf('3_1_a_%.2f_h_%g_lattice_t_res_%g_pwe_w_data_full_path.csv',lat_a,h,fr(1));
...
label_file_freq = sprintf('3_2_a_%.2f_h_%g_lattice_t_res_%g_pwe_f_data_full_path.csv',lat_a,h,fr(1));
