function unified_pwe_lrsc_plate_gamma_x_all_lattices = pwe_lrsc_gamma_x_unified_calculation(lattice_type, nmax_f, i_fi_f, i_ff_f, a_f, he_f, E_f, gamma_f, ro_f, ni_f, fr_f, eta_p_f)
%--------------------------------------------------------------------------
% UNIFIED_PWE_LRSC_PLATE_GAMMA_X_ALL_LATTICES
%--------------------------------------------------------------------------
...
%            de rede cristalina no caminho Gamma-X
%
% Entrada:
...
%
% Saida:
%   unified_pwe_lrsc_plate_gamma_x_all_lattices : Status de execucao (1 = sucesso)
%
% Autor: Anderson H.R. Ferreira
% Data: July 2025
% Versao: 2.0 - Gamma-X Path Analysis
%--------------------------------------------------------------------------
...
switch lattice_type
    case 'square'
        ...
        g1 = [b1, 0];
        g2 = [0, b2];
        % Pontos de alta simetria para Gamma-X
        gamma_point = [0, 0];
        x_point = [pi/a1, 0];
    ...
    case 'triangular'
        ...
        g1 = [b1, -b2/sqrt(3)];
        g2 = [0, 2*b2/sqrt(3)];
        % Pontos de alta simetria para Gamma-X (adaptado para triangular)
        gamma_point = [0, 0];
        x_point = [pi/a1, 0];  % Equivalente ao ponto M
        % Posicao do ressonador
    ...
    case 'hexagonal'
        ...
        g1 = [b1, -b2/sqrt(3)];
        g2 = [0, 2*b2/sqrt(3)];
        % Pontos de alta simetria para Gamma-X (adaptado para hexagonal)
        gamma_point = [0, 0];
        x_point = [pi/a1, pi/(sqrt(3)*a2)];  % Equivalente ao ponto M
    ...
    case 'kagome'
        ...
        g1 = [b1, -b2/sqrt(3)];
        g2 = [0, 2*b2/sqrt(3)];
        % Pontos de alta simetria para Gamma-X (adaptado para kagome)
        gamma_point = [0, 0];
        x_point = [pi/a1, pi/(sqrt(3)*a2)];  % Equivalente ao ponto M
...
%--------------------------------------------------------------------------
% DEFINICAO DO CAMINHO GAMMA-X NO ESPACO K
%--------------------------------------------------------------------------
% Caminho Gamma-X simplificado
k_path = [gamma_point; x_point];

% Numero de pontos no caminho Gamma-X
n_points = 100;
k_points = [];

% Interpolacao linear de Gamma para X
k_x = linspace(gamma_point(1), x_point(1), n_points);
k_y = linspace(gamma_point(2), x_point(2), n_points);
k_points = [k_x', k_y'];
...
%--------------------------------------------------------------------------
fprintf('Calculando PWE-LRSC para rede %s (Gamma-X) com fr = %.1f Hz...\n', lattice_type, fr_f);
...
% SALVAR RESULTADOS
%--------------------------------------------------------------------------
% Normalizar vetor de onda para caminho Gamma-X
...
% Gerar nomes de arquivo com frequencia do ressonador para Gamma-X
label_file_w_kk = sprintf('%d_1_a_%.2f_h_%g_lattice_k_res_%g_pwe_lrsc_w_data_gamma_x_path.csv', ...
    lattice_id, a_f, h, fr_f);
label_file_freq = sprintf('%d_2_a_%.2f_h_%g_lattice_k_res_%g_pwe_lrsc_f_data_gamma_x_path.csv', ...
    lattice_id, a_f, h, fr_f);
...
unified_pwe_lrsc_plate_gamma_x_all_lattices = 1;
