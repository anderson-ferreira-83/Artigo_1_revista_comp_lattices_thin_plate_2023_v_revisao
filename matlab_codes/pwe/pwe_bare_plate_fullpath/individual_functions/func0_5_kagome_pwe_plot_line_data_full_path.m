function func0_5_kagome_pwe_plot_line_data_full_path = kagome_dispersion(nmax_f,i_fi_f,i_ff_f,a_f,he_f,E_f,gamma_f,ro_f,ni_f,fr_f,eta_p_f)
%disp('Parametros - Rede kagome')
im = sqrt(-1);
...
%disp('Area da Celula unitaria [m^2]')
S = (lat_a^2)*(2*sqrt(3));
...
for i=-nmax:nmax
    for j=-nmax:nmax
        G = [G;(i-j)*(pi/lat_a) (i+j)*(pi/lat_a)*(1/(sqrt(3)))];
    end
end
% Trajetorias
gamma   =            [0 0];
X       = (pi/lat_a)*[2/3   0];
M       = (pi/lat_a)*[1/2   1/(2*sqrt(3))];
X_l     = (pi/lat_a)*[1/3   1/sqrt(3)];
M_l     = (pi/lat_a)*[0     1/sqrt(3)];
%----------------------
path0 = [M_l;     gamma;    X;   M;     X_l];
pathf = [gamma;   X;        M;   X_l;   M_l];
    ...
    for w=1:1
        ...
        for  ik=0:nk
            ...
            if N > 0
                ...
                % i = (length(n1)^2+N) -> Numero de linhas
                % j = ((bd=5)*N*lenght(0:nk)) -> Discretizacao da FIBZ
...
label_file_w_kk = sprintf('5_1_a_%.2f_h_%g_lattice_k_bare_pwe_w_data_full_path.csv',lat_a,h);
...
label_file_freq = sprintf('5_2_a_%.2f_h_%g_lattice_k_bare_pwe_f_data_full_path.csv',lat_a,h);
