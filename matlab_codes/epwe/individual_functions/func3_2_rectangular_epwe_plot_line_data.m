function func3_2_rectangular_epwe_plot_line_data = rectangular_dispersion(a_f,he_f,E_f,gamma_f,ro_f,ni_f,eta_p_f,fr_f)
...
for kk=2:length(f)
    ...
    c4=0;
    phi=0;             % phi=pi/6; phi=pi/5; phi=pi/4;  
    b11=2*pi/lat_ax;
    b12=0;
    b21=0;
    b22=2*pi/lat_ay;
...
label_file_q = sprintf('2_1_a_%.2f_h_%g_lattice_r_res_%g_epwe_real_w_data_gamma_x_path.csv',a1,h,fr);
...
label_file_q = sprintf('2_1_a_%.2f_h_%g_lattice_r_res_%g_epwe_imag_w_data_gamma_x_path.csv',a1,h,fr);
writematrix(imag(q(:,:)*lat_a/2/pi),label_file_q);
sourceFilePath = fullfile(pwd,label_file_q);
targetDirectory = fullfile(pwd, '1_database');
movefile(sourceFilePath, targetDirectory);

label_file_f = sprintf('2_2_a_%.2f_h_%g_lattice_r_res_%g_epwe_f_data_gamma_x_path.csv',a1,h,fr);
