function func3_3_triangular_epwe_plot_line_data = triangular_dispersion(a_f,he_f,E_f,gamma_f,ro_f,ni_f,eta_p_f,fr_f)
...
for kk=2:length(f)
    ...
    b11=2*pi/(sqrt(3)*lat_a)*sqrt(3);
    b12=2*pi/(sqrt(3)*lat_a)*(-1);
    b21=0;
    b22=4*pi/(sqrt(3)*lat_a);
    for i=1:N
...
label_file_q = sprintf('3_1_a_%.2f_h_%g_lattice_t_res_%g_epwe_real_w_data_gamma_x_path.csv',a1,h,fr);
...
label_file_q = sprintf('3_1_a_%.2f_h_%g_lattice_t_res_%g_epwe_imag_w_data_gamma_x_path.csv',a1,h,fr);
...
label_file_f = sprintf('3_2_a_%.2f_h_%g_lattice_t_res_%g_epwe_f_data_gamma_x_path.csv',a1,h,fr);
