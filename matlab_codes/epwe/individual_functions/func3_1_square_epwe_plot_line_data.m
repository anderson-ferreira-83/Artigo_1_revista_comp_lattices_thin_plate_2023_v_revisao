function func3_1_square_epwe_plot_line_data = square_dispersion(a_f,he_f,E_f,gamma_f,ro_f,ni_f,eta_p_f,fr_f)
...
%q_vec = q*lat_a/2/pi;
label_file_q = sprintf('1_1_a_%.2f_h_%g_lattice_s_res_%g_epwe_real_w_data_gamma_x_path.csv',a1,h,fr);
...
label_file_q = sprintf('1_1_a_%.2f_h_%g_lattice_s_res_%g_epwe_imag_w_data_gamma_x_path.csv',a1,h,fr);
writematrix(imag(q(:,:))*lat_a/2/pi,label_file_q);
sourceFilePath = fullfile(pwd,label_file_q);
targetDirectory = fullfile(pwd, '1_database');
movefile(sourceFilePath, targetDirectory);

label_file_f = sprintf('1_2_a_%.2f_h_%g_lattice_s_res_%g_epwe_f_data_gamma_x_path.csv',a1,h,fr);
