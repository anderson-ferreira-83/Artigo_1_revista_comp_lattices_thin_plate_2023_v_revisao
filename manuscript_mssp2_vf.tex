...
\textcolor{red}{The comprehensive individual analyses presented in previous subsections enable quantitative performance comparison across all five lattice configurations using two complementary metrics that address distinct design considerations. This dual-metric framework employs: (1) \textbf{absolute bandwidth} (FBGW in [Hz]) providing direct engineering insights for applications with specific frequency targets, and (2) \textbf{relative bandwidth} ($\eta_{rel}$ in [\%]) enabling frequency-independent geometric performance comparison through normalization. Together, these metrics establish design hierarchies based on both practical operational requirements and fundamental geometric efficiency principles. The analysis begins with absolute bandwidth characterization to understand frequency-specific performance, followed by relative bandwidth analysis to reveal intrinsic geometric advantages.}
\newpage
\begin{figure}[htb]  
	\centering  
...
\textcolor{red}{While the absolute bandwidth analysis (FBGW in [Hz]) presented above provides critical insights for engineering applications with specific frequency targets—enabling designers to select optimal configurations based on actual bandwidth required at predetermined frequencies (e.g., triangular lattice with 55.40 Hz bandwidth at 145 Hz for machinery vibration control)—this metric alone presents fundamental limitations for fair geometric comparison. Different lattices achieve their maximum performance at substantially different frequency ranges: triangular lattice peaks at 145 Hz while square lattice optimizes at 105 Hz, a 38\% frequency difference. This discrepancy potentially biases conclusions toward configurations operating at higher frequencies, where absolute bandwidth naturally increases due to frequency-dependent wave physics, obscuring intrinsic geometric performance advantages.}

\textcolor{red}{To address this limitation and enable objective comparison of fundamental geometric efficiency independent of operational frequency, this study implements relative bandwidth analysis following the methodology recommended by the metamaterial community. The relative bandgap width is defined as:
\begin{equation}
\eta_{rel} = \frac{f_2 - f_1}{f_c} \times 100\%
\label{eq:relative_bandwidth}
\end{equation}
where $f_c = (f_1 + f_2)/2$ is the center frequency of the bandgap, $f_1$ and $f_2$ are the lower and upper bandgap boundaries, and $\eta_{rel}$ represents the normalized bandwidth efficiency. This dimensionless metric removes frequency-dependent scaling effects, enabling direct comparison of intrinsic geometric performance across different operational ranges. The normalization accounts for the physical reality that bandgap width typically scales with frequency, isolating purely geometric contributions to metamaterial efficiency.}

\textcolor{red}{The comprehensive relative bandgap width analysis presented in Table \ref{tab:relative_bandwidth_comparison} applies this normalized metric across the complete frequency range investigated (10-150 Hz) for all five lattice configurations, providing rigorous frequency-independent performance comparison using the $\eta_{rel}$ metric from Equation \ref{eq:relative_bandwidth}. This normalization removes frequency-dependent bias, enabling fair comparison of intrinsic geometric performance independent of operational frequency. The complete frequency-sweep analysis reveals that triangular lattices achieve consistently superior relative bandwidth performance, reaching peak efficiency of 42.51\% compared to 31.40\% for square lattices—representing a 35\% improvement in normalized terms. The analysis demonstrates that geometric optimization maintains its advantage across the entire frequency spectrum, validating the superiority claim through rigorous normalized metrics independent of absolute frequency positioning.}
...
\multicolumn{6}{l}{\footnotesize Square: 31.40\% at 100 Hz; Rectangular: 20.41\% at 70 Hz} \\
\multicolumn{6}{l}{\footnotesize Triangular: 42.51\% at 140 Hz; Honeycomb: 23.37\% at 50 Hz; Kagomé: 20.07\% at 10 Hz} \\
\hline
\end{tabular}
}
\end{table}

\textcolor{red}{The dual-metric comparative framework (absolute + relative bandwidth) provides comprehensive design guidelines: (1) \textbf{Absolute bandwidth (FBGW)} guides frequency-specific applications where target operational range is predetermined (e.g., machinery vibration at 100-150 Hz requires triangular lattice with 55.40 Hz bandwidth at 145 Hz); (2) \textbf{Relative bandwidth ($\eta_{rel}$)} reveals intrinsic geometric efficiency, enabling material-independent and frequency-independent performance ranking (triangular 42.51\% consistently superior across all frequencies). Together, these complementary metrics address both the practical engineering question "which lattice for my target frequency?" and the fundamental scientific question "which geometry is intrinsically superior?".}
...
\textcolor{red}{The comprehensive analysis of five lattice configurations reveals fundamental distinctions between single-resonator systems (square, rectangular, triangular) exhibiting single band gap behavior and multi-resonator systems (honeycomb with 2 resonators, kagomé with 3 resonators) displaying dual complete band gaps originating from coupled resonator modes. Systematic bandwidth evolution analysis across 15 resonator frequencies (10-150 Hz) establishes quantitative performance hierarchies: triangular lattices achieve 35\% superior relative bandwidth (42.51\% vs 31.40\% for square lattices) with consistent broadband superiority, honeycomb provides balanced dual-band capability ideal for multi-frequency applications, kagomé delivers maximum low-frequency attenuation (15 dB enhancement) despite narrower bandwidth, while rectangular lattices demonstrate directional selectivity. PWE/EPWE semi-analytical methods enable this extensive parametric investigation with 1800-5700× computational efficiency over FEM while maintaining <1\% error, establishing the first complete comparative framework for lattice-based locally resonant metamaterial plates.}
