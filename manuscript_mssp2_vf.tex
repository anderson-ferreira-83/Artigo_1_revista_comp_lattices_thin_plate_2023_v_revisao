...
\begin{figure}[htb]
	\centering
	\includegraphics[width=0.7\textwidth]{ilustr_inf_panel_cell_unit-eps-converted-to.pdf}
...
\begin{figure}[htb]
	\centering
	\includegraphics[width=0.7\textwidth]{ilustr_unit_cell_type_lattice-eps-converted-to.pdf}
...
\label{eq:mass_ratio}
\end{equation}
where $m_{p,i}$ is the plate mass per unit cell for lattice configuration $i$, and $m_{p,\text{kagomé}} = 4.16 \times 10^{-2}$ kg represents the reference mass (kagomé lattice with largest unit cell area). This normalization enables direct material efficiency comparison across different lattice geometries.

This normalization reveals material efficiency differences: triangular (25%) and rectangular (14%) lattices achieve superior performance with minimal material usage compared to kagomé.} The computational implementation employs optimized discretization parameters in Table~\ref{time_process_simu_methods} to balance numerical accuracy with efficiency.
...
\textcolor{red}{Triangular lattice achieves FBGW 1 ($\Delta f_{12} = 24.24$ Hz) without partial band gaps, demonstrating isotropic wave blocking. Maximum bandwidth $\Delta f_{12} = 55.40$ Hz at $f_j = 145$ Hz represents 73\% improvement over square lattice, confirming geometric superiority.} 
...
While this study focuses on polymeric materials for practical validation through rapid prototyping, the PWE/EPWE methodology developed is applicable to a broad range of structural materials. This appendix demonstrates the universality of our framework by extending the analysis to metallic materials (aluminum and steel), revealing that the geometric advantages identified for different lattice configurations represent material-independent design principles.

\subsection{Material Properties and Scaling Analysis}

The choice of Vero White Plus polymeric material was strategically motivated by rapid prototyping capabilities through 3D printing, enabling precise fabrication of complex lattice geometries with controlled tolerances for experimental validation. However, structural engineering applications often require metallic materials with significantly higher bending stiffness, necessitating analysis across multiple material scales.
...
\begin{table}[htb]
    ...
    \begin{tabular}{|l|c|c|c|c|c|}
        \hline
        \textbf{Material} & \textbf{E [GPa]} & \textbf{ρ [kg/m³]} & \textbf{ν} & \textbf{D [N⋅m]} & \textbf{D/D$_{polymer}$} \\
        \hline
        Vero White Plus & 0.86 & 600 & 0.36 & 1.22×10$^{-6}$ & 1.0 \\
        \hline
        Aluminum 6061 & 69 & 2700 & 0.33 & 1.17×10$^{-5}$ & 9.6 \\
        \hline
        Steel AISI 1020 & 200 & 7850 & 0.29 & 3.39×10$^{-4}$ & 278 \\
        \hline
    \end{tabular}
    \label{tab:material_properties_comparison}
\end{table}

The bending stiffness D = Eh³/12(1-ν²) scales dramatically across materials, with steel exhibiting 278× higher rigidity than the polymer. This scaling directly affects the operational frequency ranges while maintaining the validity of Kirchhoff-Love thin plate assumptions (h/a = 0.02 ≪ 0.1).

\subsection{Frequency Scaling and Operational Ranges}

The characteristic frequency scaling follows the relationship f$_B$ = (1/2π)(π/a)√(D/ρh), resulting in proportional frequency shifts across materials while preserving geometric relationships. Table \ref{tab:frequency_scaling} summarizes the operational frequency ranges for each material:
...
\begin{table}[htb]
    ...
    \begin{tabular}{|l|c|c|c|c|c|}
        \hline
        \textbf{Material} & \textbf{h [mm]} & \textbf{a [mm]} & \textbf{h/a} & \textbf{Target Range [Hz]} & \textbf{f$_B$ [Hz]} \\
        \hline
        Vero White Plus & 2.0 & 100 & 0.02 & 10-200 & 120 \\
        \hline
        Aluminum 6061 & 2.0 & 100 & 0.02 & 200-600 & 370 \\
        \hline
        Steel AISI 1020 & 2.0 & 100 & 0.02 & 400-1000 & 580 \\
        \hline
    \end{tabular}
    \label{tab:frequency_scaling}
\end{table}

\subsection{PWE Analysis Results for Metallic Materials}

Using identical PWE computational parameters (M = 2, convergence tolerance 10$^{-6}$), the band gap analysis was extended to aluminum and steel configurations. The methodology maintains numerical stability across the 278× stiffness variation, confirming the robustness of the semi-analytical approach.

\subsubsection{Aluminum Analysis (200-600 Hz Range)}

Table \ref{tab:aluminum_results} presents the complete band gap width analysis for aluminum lattices:

\begin{table}[htb]
    \small
    \centering
    \caption{Band gap performance for Aluminum 6061 lattices}
    \begin{tabular}{|l|c|c|c|c|}
        \hline
        \textbf{Lattice Type} & \textbf{Max FBGW [Hz]} & \textbf{Frequency [Hz]} & \textbf{Relative BW [\%]} & \textbf{Performance Rank} \\
        \hline
        Triangular & 168.2 & 435 & 38.6 & 1st \\
        \hline
        Square & 97.5 & 315 & 30.9 & 2nd \\
        \hline
        Honeycomb & 87.1 & 410 & 21.2 & 3rd \\
        \hline
        Rectangular & 62.3 & 297 & 20.9 & 4th \\
        \hline
        Kagomé & 20.8 & 303 & 6.9 & 5th \\
        \hline
    \end{tabular}
    \label{tab:aluminum_results}
\end{table}

\subsubsection{Steel Analysis (400-1000 Hz Range)}

Table \ref{tab:steel_results} presents the complete band gap width analysis for steel lattices:

\begin{table}[htb]
    \small
    \centering
    \caption{Band gap performance for Steel AISI 1020 lattices}
    \begin{tabular}{|l|c|c|c|c|}
        \hline
        \textbf{Lattice Type} & \textbf{Max FBGW [Hz]} & \textbf{Frequency [Hz]} & \textbf{Relative BW [\%]} & \textbf{Performance Rank} \\
        \hline
        Triangular & 278.5 & 720 & 38.7 & 1st \\
        \hline
        Square & 161.2 & 522 & 30.9 & 2nd \\
        \hline
        Honeycomb & 144.3 & 680 & 21.2 & 3rd \\
        \hline
        Rectangular & 103.1 & 492 & 20.9 & 4th \\
        \hline
        Kagomé & 34.4 & 502 & 6.9 & 5th \\
        \hline
    \end{tabular}
    \label{tab:steel_results}
\end{table}

\subsection{Universal Performance Hierarchy}

The comparative analysis across three materials reveals a remarkable consistency in lattice performance hierarchy, as summarized in Table \ref{tab:universal_hierarchy}:

\begin{table}[htb]
    \small
    \centering
    \caption{Universal lattice performance hierarchy across materials}
    \begin{tabular}{|l|c|c|c|}
        \hline
        \textbf{Lattice Ranking} & \textbf{Vero+ (10-200Hz)} & \textbf{Al (200-600Hz)} & \textbf{Steel (400-1000Hz)} \\
        \hline
        1st: Triangular & 55.40 Hz (35.2\%) & 168.2 Hz (38.6\%) & 278.5 Hz (38.7\%) \\
        \hline
        2nd: Square & 32.10 Hz (28.4\%) & 97.5 Hz (30.9\%) & 161.2 Hz (30.9\%) \\
        \hline
        3rd: Honeycomb & 28.67 Hz (20.9\%) & 87.1 Hz (21.2\%) & 144.3 Hz (21.2\%) \\
        \hline
        4th: Rectangular & 20.53 Hz (20.7\%) & 62.3 Hz (20.9\%) & 103.1 Hz (20.9\%) \\
        \hline
        5th: Kagomé & 6.85 Hz (6.8\%) & 20.8 Hz (6.9\%) & 34.4 Hz (6.9\%) \\
        \hline
    \end{tabular}
...
2. \textbf{Frequency scaling is predictable:} Performance ratios scale proportionally with √(D/ρh), enabling systematic material selection for target frequencies.
