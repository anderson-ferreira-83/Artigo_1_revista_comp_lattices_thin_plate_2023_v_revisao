...
Despite their effectiveness, PCs face a fundamental limitation for low-frequency applications: Bragg's condition \(a = n\lambda/2\) necessitates large unit cells to attenuate low-frequency waves \cite{Laude2015}, challenging compact device design, particularly for flexural \cite{Hussein2006} or elastic waves in complex media \cite{Zhang2014}.
...
Building upon these developments, Xiao et al. investigated flexural wave propagation in thin plates with periodic spring--mass resonators using EPWE \cite{Xiao_2012}, revealing the coexistence of Bragg-type and locally resonant gaps, as well as wide pseudo-gaps dependent on resonator natural frequency. Their subsequent work demonstrated that beam-like resonators periodically attached to plates can induce low-frequency complete band gaps for flexural waves \cite{Xiao_2014}, with tunable resonator properties allowing significant control over band gap location and width.
...
This section validates the theoretical research established in Section~\ref{formu_mat} through systematic analysis of five lattice configurations that span from single-resonator architectures (square, rectangular, triangular) to multi-resonator systems (honeycomb, kagomé). The investigation addresses three critical aspects: (i) lattice geometry influence on bandgap formation through unit cell symmetry and area effects; (ii) optimal resonator configuration via single vs. multi-resonator performance comparison; and (iii) theoretical-to-practical translation through PWE-FEM validation. All configurations employ physically realizable parameters optimized for low-frequency applications (10-200 [Hz]), emphasizing 3D printable polymer materials to ensure experimental feasibility and consistent cross-lattice comparison. The material parameters follow the experimental characterization of Vero White Plus polymer manufactured via 3D printing with UV curing technology, as validated in \cite{MIRANDA2019480}. The material and geometric parameters in Table~\ref{param_geo_struc_cell_unit} enable systematic performance evaluation while maintaining manufacturing constraints:
...
The mass ratio $m_{\text{ratio}}$ normalizes all configurations relative to the kagomé lattice (largest unit cell area), revealing significant material efficiency differences. The computational implementation employs optimized discretization parameters in Table~\ref{time_process_simu_methods} to balance numerical accuracy with efficiency.
...
The validation demonstrates excellent agreement with average relative error of $0.68\% \pm  0.24\%$ across all configurations and frequency points. The Root Mean Square Error (RMSE) values range from $0.89$ [Hz] (triangular) to $1.67$ [Hz] (kagomé), confirming PWE accuracy within engineering tolerance while achieving computational efficiency gains of 1800-5700× over FEM.

The analysis proceeds systematically: Subsection 3.1 examines single-resonator lattices (square, rectangular, triangular) for baseline performance, Subsection 3.2 investigates multi-resonator systems (honeycomb, kagomé) for advanced coupling effects, and Subsection 3.3 provides comparative synthesis with design guidelines.

%---------------------------------------------------------
\subsection{Band structures for square, rectangular and triangular SR-SDOF lattices}
\label{srt_disp_pwe}
This subsection investigates single-resonator Bravais lattices to establish fundamental understanding of locally resonant metamaterial behavior and validate theoretical predictions. Each configuration contains a centrally positioned SR-SDOF resonator at $\mathbf{r} = (0, 0)$, creating the simplest possible metamaterial architecture where band gap formation mechanisms can be clearly identified and analyzed.

Physical foundation and geometric considerations indicate that the three lattice types represent distinct symmetry classes with profound implications for wave scattering behavior. The square lattice ($a_1 = a_2 = a$) exhibits four-fold rotational symmetry—meaning the structure remains invariant under 90° rotations—creating equivalent wave propagation characteristics along orthogonal directions. The rectangular lattice introduces geometric anisotropy through aspect ratio $a_1/a_2 = 2$, where unequal lattice parameters break rotational symmetry and create direction-dependent wave propagation. The triangular lattice provides six-fold rotational symmetry—invariance under 60° rotations—with optimal packing efficiency, enabling the highest degree of isotropy among two-dimensional Bravais lattices. These geometric differences fundamentally alter the coupling between local resonator modes and the host plate's flexural wave modes, leading to dramatically different metamaterial performance.

The FIBZ path \(M \longrightarrow \Gamma \longrightarrow X \longrightarrow M\) for square and rectangular lattices captures the essential wave propagation directions that determine band gap formation. The resonator stiffness values \(k_j = 1516\) [N/m] (square) and \(k_j = 758\) [N/m] (rectangular) are calibrated to achieve identical natural frequencies \(f_j = 80\) [Hz], enabling direct comparison of geometric effects on metamaterial behavior.
...
The Bragg scattering mechanism manifests at point $C_s$, where geometric periodicity creates wave interference independent of local resonance. The simultaneous presence of both mechanisms creates the rich modal landscape observed, with PBGW 1 ($\Delta f_{12} = 32.33$ [Hz]) and PBGW 2 ($\Delta f_{12} = 31.09$ [Hz]) representing directional band gaps where complete wave blocking occurs only along specific crystallographic directions.

Critical physical insight reveals that the local resonator fundamentally alters the flexural wave dispersion by introducing hybridization between resonator modes and plate modes. This coupling creates avoided crossings in the dispersion diagram, where modes that would otherwise intersect are pushed apart, generating the observed band gaps. The strength of this hybridization depends critically on the spatial overlap between resonator motion and the local plate deformation field.

The comprehensive parametric study across $f_j = 10-150$ [Hz] reveals fundamental scaling relationships governing metamaterial performance. This frequency range encompasses the transition from mass-loading dominated regime (low frequencies) where the resonator behaves as added inertia, through the optimal coupling regime where resonance hybridization maximizes band gap formation, to the stiffness-dominated regime (high frequencies) where the resonator approaches rigid body behavior.

The $1$ [Hz] discretization refinement using PWE enables identification of critical coupling points where slight frequency adjustments produce dramatic changes in band gap width. This high resolution analysis reveals the frequency selectivity inherent in locally resonant metamaterials, where optimal performance occurs within narrow frequency windows determined by the balance between resonator-plate coupling strength and geometric dispersion effects.

Physical interpretation of parametric behavior shows that as $f_j$ increases from low values, the resonator transitions from a passive mass element to an active wave scatterer. The optimal frequency represents the point where resonator motion achieves maximum spatial correlation with the incident flexural wave field, creating the strongest possible destructive interference. Beyond this optimum, the resonator motion becomes increasingly decoupled from the plate motion, reducing wave scattering efficiency and narrowing the achievable band gaps.
...
Figure \ref{pwe_disp_square_all_res}a) ($f_j = 10$ [Hz]) demonstrates the mass-loading regime where the resonator acts primarily as added inertia, producing minimal band gap width ($\Delta f_{12} = 2.26$ [Hz]) due to weak resonator-plate coupling. The dispersion closely follows the bare plate behavior (black line), indicating that the resonator has not yet achieved significant wave scattering capability.

Figure \ref{pwe_disp_square_all_res}b) ($f_j = 105$ [Hz]) captures the optimal coupling regime where maximum band gap width ($\Delta f_{12} = 32.10$ [Hz]) is achieved. Here, the resonator frequency precisely matches the condition for maximum energy transfer between local oscillations and propagating flexural modes. The dramatic departure from bare plate dispersion indicates strong wave-resonator interaction, with the system achieving impedance mismatch that blocks wave transmission most effectively.

Figure \ref{pwe_disp_square_all_res}c) ($f_j = 150$ [Hz]) represents the stiffness-dominated regime where the resonator frequency exceeds the optimal coupling window. The reduced band gap width ($\Delta f_{12} = 14.33$ [Hz]) reflects weakened resonator-plate interaction as the resonator motion becomes increasingly isolated from the plate's flexural behavior.
...
The rectangular geometry imposes fundamental limitations on metamaterial performance through two distinct physical mechanisms. The reduced FBGW 1 ($\Delta f_{12} = 17.74$ [Hz] vs. $23.16$ [Hz] for square) reflects weakened resonator-plate coupling due to the smaller unit cell area. Since resonator mass scales as $m_{r,j} = \gamma \rho S h$, the $50\%$ area reduction directly decreases the effective coupling strength between local oscillations and propagating flexural waves.

The anisotropic dispersion created by the rectangular aspect ratio ($a_1/a_2 = 2$) introduces directional dependence in wave propagation, where different crystallographic directions exhibit distinct scattering properties. This anisotropy explains why PBGW 1 and PBGW 2 ($\Delta f_{12} = 33.70$ [Hz] and $32.64$ [Hz]) remain nearly unchanged compared to the square lattice – these represent directional band gaps that depend more on lattice periodicity than on local resonance strength.

The mode shapes at points $A_r$, $B_r$, and $C_r$ (Figure \ref{pwe_fem_disp_modal_rect}b) reveal that geometric constraints do not eliminate the fundamental resonance mechanism but rather reduce its efficiency. Point $B_r$ continues to demonstrate local resonator influence, confirming that the wave attenuation physics remains intact despite geometric modifications.

The rectangular lattice demonstrates that unit cell area directly governs resonator effectiveness, establishing a fundamental design relationship where metamaterial performance scales with the spatial extent of resonator-plate interaction.

The rectangular lattice parametric analysis reveals accelerated optimization dynamics compared to the square configuration. The optimal frequency shifts to $f_j = 99$ [Hz] (6 [Hz] lower than square), indicating that geometric constraints alter the resonator-plate coupling landscape, requiring adjustment of the resonance frequency to maintain optimal wave scattering conditions.

The maximum bandwidth reduction to $\Delta f_{12} = 20.53$ [Hz] ($36\%$ decrease from square lattice) quantifies the geometric penalty imposed by unit cell area reduction. This relationship establishes that metamaterial performance is not merely a function of resonator properties but depends critically on the spatial platform provided by the unit cell geometry.
\newpage
\begin{figure}[htb]
	\centering
...
The triangular configuration represents optimal geometric packing in two-dimensional space, providing six-fold rotational symmetry that fundamentally enhances wave-resonator coupling compared to square and rectangular geometries. The FIBZ path \( M' \rightarrow \Gamma \rightarrow X \rightarrow M \rightarrow X' \rightarrow M' \) samples the maximally symmetric k-space directions, capturing the full range of wave propagation behaviors in this high-symmetry lattice.

Despite having a $13\%$ smaller unit cell area ($S = 0.87 \times 10^{-2}$ $m^2$ vs. $1.00 \times 10^{-2}$ $m^2$ for square), the triangular lattice achieves superior metamaterial performance through geometric efficiency. The six-fold symmetry creates multiple equivalent wave scattering pathways, providing more opportunities for resonator-wave interaction than the four-fold symmetry of square lattices. The resonator stiffness \( k_j = 1313 \) N/m maintains the standard frequency \( f_j = 80 \) [Hz] for direct comparison.
...
Figure \ref{pwe_fem_disp_modal_trian}a) reveals the absence of partial band gaps above or below FBGW 1, indicating that the six-fold symmetry creates more isotropic wave propagation compared to the directional asymmetries observed in square and rectangular lattices. This isotropy leads to complete band gaps that block waves in all directions simultaneously, representing a significant advantage for practical applications.

The similar FBGW 1 bandwidth to the square lattice at $f_j = 80$ [Hz] ($\Delta f_{12} = 24.24$ [Hz] vs. $23.16$ [Hz]) demonstrates that the triangular lattice achieves equivalent baseline performance despite its smaller unit cell area. This indicates superior area-normalized efficiency that will manifest more dramatically in the parametric analysis.

Point $B_t$ in Figure \ref{pwe_fem_disp_modal_trian}b) confirms the preserved local resonance mechanism, but the six-fold symmetric environment provides multiple equivalent coupling pathways that enhance the overall wave scattering efficiency.

The most remarkable characteristic of the triangular lattice emerges in its parametric response (Figures \ref{pwe_disp_trian_all_res}a-c), where the system exhibits dramatically enhanced bandwidth growth and exceptional stability after reaching peak performance. This behavior represents a qualitative improvement over square and rectangular configurations, indicating fundamental advantages of the triangular geometry. 

The triangular lattice parametric analysis reveals breakthrough performance that establishes this geometry as the optimal single-resonator metamaterial architecture. Figure \ref{pwe_disp_trian_all_res}a) ($f_j = 10$ [Hz]) shows typical low-frequency behavior, while Figure \ref{pwe_disp_trian_all_res}b) ($f_j = 145$ [Hz]) captures the remarkable peak performance where the triangular lattice achieves its maximum bandwidth.
...
Both band gaps converging to similar 7 [Hz] widths represents a performance ceiling imposed by three-fold symmetry. Unlike dual-resonator systems where anti-phase and in-phase modes can be independently optimized, the kagomé's triangular constraint forces all three resonators into competing phase relationships, preventing any single mode from achieving maximum coupling efficiency.

The detailed tabular analysis that follows quantifies this frustration-limited behavior across the complete frequency spectrum. Table \ref{tab_kag_latice_fbgw1} presents FBGW 1 results (modes \( f_3 \) and \( f_4 \)):
...
The infinite domain analysis has established fundamental performance hierarchies among the five lattice configurations, with quantified metrics including band gap widths, efficiency ratios, and computational advantages. Key findings include: (i) triangular lattices achieving 40\% wider band gaps than square configurations; (ii) kagomé lattices providing maximum attenuation through triple-resonator coupling; (iii) PWE/EPWE methods delivering 1800-5700× computational efficiency over FEM with <1\% error. However, practical engineering applications require understanding how these infinite-domain predictions manifest in finite structures with realistic boundary conditions. The following section bridges this theoretical-practical gap by analyzing receptance behavior in finite LRSC plates, examining how boundary effects modify the predicted band gap characteristics and establishing direct correlations between infinite-domain theoretical predictions and measurable vibration attenuation in finite structures.
 
%%%%%%%%%%%%%%%%%%%%%%%
\section{Vibration receptance of the LRSC plate}\label{simul_trans}
Building upon the infinite domain performance hierarchies established in Section \ref{num_ex_disc}—where triangular lattices demonstrated 40\% superior bandwidth, kagomé configurations achieved maximum attenuation through triple-resonator coupling, and honeycomb designs provided balanced dual-mode operation—this section investigates how these theoretical predictions manifest in finite plate structures with realistic boundary conditions. The transition from infinite periodicity to finite dimensions introduces boundary-induced mode coupling effects that can significantly modify the predicted band gap characteristics, as observed in similar metamaterial studies \cite{Xiao_2012, Li2021, Liu2018, Yan2022}.
